...
csv_file="./data/arrays_table.csv"

# parse the table columns into 3 arrays
...
# populate the array
for((i=1; i<$nlines; i++)); do column_3[$i]=$((column_2[$i] - column_1[$i]))
done
echo "${column_3[@]}"
...
for ((i-1; i<nlines; i++)); do echo "${column_3[$1]}" >> column_3.txt
done
